...
def pretty_format_dict():
    ...
    try:
        ...
        MULTIPLIERS = {}
    return json.dumps(MULTIPLIERS, indent=4)
...
"output recognition": """```Output recognition

Typing anything in Japanese in the allowed channels will give you points. How many points you will get through outputting is decided by the multipliers.
You can see them by doing /help then clicking on BOT. The multipliers are subject to change. More moderating will be focused on Japanese output to avoid abusing potential bugs.```"""
}